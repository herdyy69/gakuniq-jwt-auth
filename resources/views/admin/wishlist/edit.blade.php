...
@section('content')
    <div class="container-fluid">
        <form action="{{ route('wishlist.update', $wishlists->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="col-lg-8">
                <div class="card mb-4 shadow-lg rounded card" style="margin: 2%; padding:1% ">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="mb-0">Data wishlist</h4>
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select name="user_id" class="form-control @error('user_id') is-invalid @enderror">
                                @foreach ($users as $user)
                                    @if (old('user_id', $user->id) == $wishlists->user->id)
                        ...
                        <div class="mb-3">
                            ...
                            <select name="produk_id" class="form-control @error('produk_id') is-invalid @enderror">
                                @foreach ($produks as $produk)
                                    @if (old('produk_id', $produk->id) == $wishlists->produk->id)
